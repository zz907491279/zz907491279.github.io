...
// 函数库
 //obj=document 范围 是select的父级元素 select是选择器 功能：封装了window.onload以及查找元素或者集合的方法
	...
	function xuanxiangka1(btn,con,xiahua){
		// for 循环中把let换为var的写法 btn[i].ind=i;保证btn中的i与con中的i一致 
		for(var i=0;i<btn.length;i++){
			btn[i].ind=i;
			btn[i].onmouseover=function(){	
				...
				con[this.ind].style.display="block";
				if(i==0){
					animate(xiahua,{left:-2},500);
				
				}else{
					animate(xiahua,{left:52},500);
				}
...
function gundong(cedaohang,cdhys,neirong,topnav){
	...
	// const foot_db=$(".ybt")[0];
	// foot_db.onclick=function(){
	// 	obj.scrollTop=head.offsetTop+"px";
	// }
		...
		for(let i=0;i<section.length;i++){
			if(obj.scrollTop+ch*0.5>=section[i].offsetTop){
				for(let j=0;j<li.length;j++){
					li[j].style.background="rgb(145, 136, 136)";
				}
				li[i].style.background="rgb(215, 11, 28)";
			}else{
				li[i].style.background="rgb(145, 136, 136)";
			}
		}
